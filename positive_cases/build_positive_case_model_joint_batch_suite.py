...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "report variants, joint-20 elicitation only, with joint direct and "
            "joint reasoning requests combined into one file per model."
        )
    )
    parser.add_argument(
        "--df-pgg",
...
def build_joint_system_prompt(include_reasoning: bool) -> str:
...
def build_requests(
    ...
    requests: list[dict] = []
    reasoning_only = _is_o_series(model)
    ...
    if not reasoning_only:
        requests.append(
            build_openai_request(
                ...
                model=model,
                system_prompt=build_joint_system_prompt(include_reasoning=False),
    ...
    requests.append(
        build_openai_request(
            ...
            model=model,
            system_prompt=build_joint_system_prompt(include_reasoning=True),
    ...
    for payload in variant_payloads:
            ...
            requests.append(
                build_openai_request(
                    ...
                    user_prompt=wrap_with_report(
                        report_text,
                        build_joint_prompt(df, include_reasoning=False),
                    ),
                    include_reasoning=False,
                )
            )
        requests.append(
            build_openai_request(
                custom_id=f"{variant_name}_joint_reasoning",
                model=model,
                system_prompt=build_joint_system_prompt(include_reasoning=True),
                user_prompt=wrap_with_report(
                    report_text,
                    build_joint_prompt(df, include_reasoning=True),
                ),
                include_reasoning=True,
            )
        )
...
def main() -> None:
    ...
    for model in args.models:
        ...
        output_path = args.output_dir / f"prediction_positive_case_variants_joint_{model_tag}.jsonl"
